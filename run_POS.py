...
over POS Tagging on the Universal Dependences English datasets
"""
...
from readers.universaldependencydatasetreader import *
...
def add_arguments(parser):
    ...
    config = [
        ...
        ["N_LAYERS", 3, int, False, "number of hidden layers for both the task model and the domain classifier"],
        ["DATASET_READER", None, str, True, "Registered Allenlp dataset reader for this dataset"], # required
        ["DROPOUT", 0.5, float, False, "value of dropout"],
        # Domain Discriminator Model Params  all start by DD
        ["DD_FIX_WEIGHTS", True, bool, False, "fix input layers of the Domain classifier or not"],
        ["DD_HIDDEN_DIM", 250, int, False, "dimension of hidden layer"],
        ["DD_EXTRA_N_LAYERS", 1, int, False, "number of extra hidden layers for the Domain classifier"],
        ["DD_EXTRA_RNN_LAYERS", 1, int, False, "number of extra hidden layers for the Domain classifier"],
        ["DD_DATASET_READER", None, str, True, "Allennlp Reader of for domain discrimination"],
        ["DD_DROPOUT", 0.5, float, False, "value of dropout"],

        # Training Params
        ["BATCH_SIZE", 128, int, False, "Batch size"],
        ["PATIENCE", 5, int, False, "patience"],
        ...
        ["DD_BATCH_SIZE", 128, int, False, "Batch size"],
        ["DD_PATIENCE", 10, int, False, "patience"],
        ["DD_EPOCHS", 30, int, False, "epochs"],
        ["DD_LR", 0.01, float, False, "learning rate"],
        ["DD_OPTIM", "adam", str, False, "optimizer type"],
        ["DD_GRAD_CLIP", 3, float, False, "value of gradient clipping threshold"],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='train a POS tagger with domain shift over UD datasets')
    ...
    vocab = Vocabulary.from_instances(d1_train_dataset + d1_valid_dataset + d2_train_dataset + d2_valid_dataset, max_vocab_size=30000)

    dd_vocab = Vocabulary.from_instances(dd_train_dataset + dd_valid_dataset, max_vocab_size=30000)
    # copying token2index from vocab to dd_vocab
    # to ensure that tokens are getting the correct index to be sent to the encoder for domain classification task
    dd_vocab._token_to_index["tokens"] = vocab._token_to_index["tokens"]
    dd_vocab._index_to_token["tokens"] = vocab._index_to_token["tokens"]
    ...
    # MODEL
    reset_seed(args.SEED)
    model = create_model(vocab, args.EMBEDDING_DIM, args.HIDDEN_DIM, TaskModel=BaseSequenceTagger,
                         encoder_type=args.ENCODER_RNN, bidirectional=True,
                         wemb=args.W_EMB, dropout=args.DROPOUT, num_layers=args.N_LAYERS)
    ...
    calib_trainer = Trainer(model=model_calibrator,
                            ...
                            validation_dataset=d1_train_dataset,
                            patience=args.PATIENCE,
                            num_epochs=args.EPOCHS,
                            cuda_device=args.GPU,
                            serialization_dir=None,
    ...
    reset_seed(args.SEED)
    dd_model = create_model(dd_vocab, args.EMBEDDING_DIM, args.HIDDEN_DIM, TaskModel=SequenceDomainClassifier,
                            encoder_type=args.ENCODER_RNN,
                            num_extra_layers=args.DD_EXTRA_N_LAYERS,
                            num_extra_rnn_layers=args.DD_EXTRA_RNN_LAYERS,
                            bidirectional=True,
                            extra_rnn_hidden_size=args.DD_HIDDEN_DIM,
    ...
    dd_iterator.index_with(dd_vocab)
    ...
    print("""
    ##########################################
    # Optimizing Optimal domain discriminator#
    ##########################################
    """)

    # Domain discriminator Model
    reset_seed(args.SEED)
    g_dd_model = create_model(dd_vocab, args.EMBEDDING_DIM, args.HIDDEN_DIM, TaskModel=SequenceDomainClassifier,
                            encoder_type=args.ENCODER_RNN,
                            num_extra_layers=args.DD_EXTRA_N_LAYERS,
                            num_extra_rnn_layers=args.DD_EXTRA_RNN_LAYERS,
                            bidirectional=True,
    ...
    g_dd_iterator.index_with(dd_vocab)
    ...
    predictor = SequenceTaggingPredictor(model=model, dataset_reader=r_reader)
    ...
    r_opt_d1_train_dataset = reader.read(d1_train_dataset_path, annotator=predictor)
    ...
    trainer = Trainer(model=r_opt_model,
                      ...
                      iterator=iterator,
                      train_dataset=r_opt_d1_valid_dataset,
                      validation_dataset=r_opt_d1_train_dataset,
                      patience=args.PATIENCE,
                      num_epochs=args.EPOCHS,
                      cuda_device=args.GPU,
                      serialization_dir=None,
    ...
    d1_on_d1_test_metrics = evaluate(model, d1_test_dataset, iterator, GPU)  # classifier suffering domain shift
    ...
    # d1d2d1 RCA
    r_test_metrics = evaluate(r_model, d1_test_dataset, iterator, GPU)
    # d1d1d1 RCA
    r_opt_test_metrics = evaluate(r_opt_model, d1_test_dataset, iterator, GPU)
    ...
    all_test_metrics = [[d1_train_dataset_path, d2_train_dataset_path,
                         ...
                         r_test_metrics, r_opt_test_metrics]]
    ...
    for d1_name, d2_name, \
        ...
        s += "d1: %s\n" % d1_train_dataset_path \
             ...
             + "PARAM: %s\n" % str([args.W_EMB, args.SEED, args.HIDDEN_DIM, args.N_LAYERS, args.DROPOUT, args.LR]) \
